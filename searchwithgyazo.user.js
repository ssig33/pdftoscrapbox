...
(function() {
  ...
  const searchArticleAndAppend = async ({image_id, thumb_url}, urls)=>{
    ...
    if(data.pages.length > 0){
      data.pages.forEach((page)=>{
        if(urls.indexOf(page.title) > -1) return true;
        urls.push(page.title)
        const li = jQuery(`
          <li class="page-list-item list-style-item">
            <a href="/${prj}/${encodeURIComponent(page.title)}" rel="route">
              <div class="icon">
                <img loading="lazy" src="${thumb_url}">
              </div>
              <div class="content">
  ...
  const search = async ()=>{
    ...
    const urls = [];
    data.captures.forEach(cap => searchArticleAndAppend(cap, urls))
